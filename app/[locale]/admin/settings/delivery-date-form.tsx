        ...
        <div className='space-y-4'>
          {fields.map((field, index) => (
            <div key={field.id} className='flex gap-2'>
              ...
              <FormField
                control={form.control}
                name={`availableDeliveryDates.${index}.deliveryCharge`}
                render={({ field }) => (
                  <FormItem>
                    {index == 0 && <FormLabel>Delivery Charge</FormLabel>}
                    <FormControl>
                      <Input {...field} placeholder='deliveryCharge' />
                    </FormControl>
                    <FormMessage>
                      {
                        errors.availableDeliveryDates?.[index]?.deliveryCharge
                          ?.message
                      }
                    </FormMessage>
                  </FormItem>
              ...
              <FormField
                control={form.control}
                name={`availableDeliveryDates.${index}.freeDeliveryMinCharge`}
                render={({ field }) => (
                  <FormItem>
                    {index == 0 && <FormLabel>Free Delivery</FormLabel>}
                    <FormControl>
                      <Input {...field} placeholder='freeDeliveryMinCharge' />
                    </FormControl>
                    <FormMessage>
                      {
                        errors.availableDeliveryDates?.[index]
                          ?.freeDeliveryMinCharge?.message
                      }
                    </FormMessage>
                  </FormItem>
                )}
